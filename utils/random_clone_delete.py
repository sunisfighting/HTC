...
def add_and_remove_edges(G, p_remove_connection, num_remove=10):
    ...
    #             if len(list(G.neighbors(remove))) > 1:
...
def parse_args():
    ...
    parser.add_argument('--input', default="./graph_data/bn/graphsage", help='Path to load data')
    parser.add_argument('--output', default="./graph_data/bn", help='Path to save data')
    parser.add_argument('--prefix', default="econ", help='Dataset prefix')
    parser.add_argument('--pdel', type=float, default=0.5, help='Probability of remove old edges')
    parser.add_argument('--ndel', type=float, default=0.5, help='Number of removed edges')
...
def main(args):
    ...
    # id2idx_file = os.path.join(args.input, 'id2idx.json')
    ...
    H = G.copy()
    e = len(G.edges())
    print(int(args.ndel * e))
    rem_edges = add_and_remove_edges(H, args.pdel, int(args.ndel * e))
