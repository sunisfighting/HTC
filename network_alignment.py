...
def parse_args():
    ...
    subparsers = parser.add_subparsers(dest="algorithm", help='Choose 1 of the algorithm from: IsoRank, FINAL, UniAlign, NAWAL, DeepLink, REGAL, IONE, PALE, HTC')
    ...
    #HTC
    parser_MyAlign = subparsers.add_parser("HTC", help="HTC algorithm")
    ...
    parser_MyAlign.add_argument('--k', type=int, default=200)
    parser_MyAlign.add_argument('--ulr', type=float, default=0.001)
    parser_MyAlign.add_argument('--flr', type=float, default=0.01)
    parser_MyAlign.add_argument('--alpha', type=float, default=1.3)
    parser_MyAlign.add_argument('--p', type=float, default=0.0)


    # DATAlign
    parser_DATAlign = subparsers.add_parser("DATAlign", help="DATAlign algorithm")
    parser_DATAlign.add_argument('--cuda',                action="store_true")
    parser_DATAlign.add_argument('--embedding_dim',       default=200,         type=int)
    parser_DATAlign.add_argument('--DATAlign_epochs',    default=20,        type=int)
    parser_DATAlign.add_argument('--lr', default=0.01, type=float)
    parser_DATAlign.add_argument('--num_GCN_blocks', type=int, default=2)
    parser_DATAlign.add_argument('--act', type=str, default='tanh')
    parser_DATAlign.add_argument('--log', action="store_true", help="Just to print loss")
    parser_DATAlign.add_argument('--invest', action="store_true", help="To do some statistics")
    parser_DATAlign.add_argument('--input_dim', default=100, help="Just ignore it")
    parser_DATAlign.add_argument('--train_dict')
    parser_DATAlign.add_argument('--alpha0', type=float, default=1)
    parser_DATAlign.add_argument('--alpha1', type=float, default=1)
    parser_DATAlign.add_argument('--alpha2', type=float, default=1)
    parser_DATAlign.add_argument('--source_embedding')
    parser_DATAlign.add_argument('--target_embedding')
    parser_DATAlign.add_argument('--noise_level', type=float, default=0.01)

    # refinement
    parser_DATAlign.add_argument('--refinement_epochs', default=10, type=int)
    parser_DATAlign.add_argument('--threshold_refine', type=float, default=0.94, help="The threshold value to get stable candidates")

    # loss
    parser_DATAlign.add_argument('--beta', type=float, default=0.8, help='balancing source-target and source-augment')
    parser_DATAlign.add_argument('--threshold', type=float, default=0.01, help='confidence threshold for adaptivity loss')
    parser_DATAlign.add_argument('--coe_consistency', type=float, default=0.8, help='balancing consistency and adaptivity loss')
...
if __name__ == '__main__':
    ...
    if algorithm == "IsoRank":
    ...
    elif algorithm == "DATAlign":
        model = DATAlign(source_dataset, target_dataset, args)
    elif algorithm == "PALE":
        ...
        model = HTC(source_dataset, target_dataset, groundtruth, args)
    else:
    ...
    for i in range(len(S)):
        print("-"*100)
        acc, MAP, top5, top10 = get_statistics(S[i], groundtruth, use_greedy_match = args.gm, get_all_metric=True)
