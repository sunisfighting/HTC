...
class GCN(nn.Module):
    ...
    """
    def __init__(self, activate_function, input_dim, output_dim, num_orbits, num_nodes):
        """
        ...
        self.A_weight = nn.Parameter(torch.rand((num_orbits, num_nodes, num_nodes)))
        init_weight(self.modules(), activate_function)
    
    def forward(self, input, A_orbits): #A_是多个orbit的拉普拉斯矩阵
        # H^(l-1) * W
        output = self.linear(input)
        # C * H^(l-1) * W
        A_hat = torch.mul(A_orbits, self.A_weight)
...
class DAT_Align(nn.Module):
    ...
    def forward(self, A_hat, net='s', new_feats=None):
        ...
        emb_input = input.clone()  #共享梯度不共享内存
        outputs = [emb_input]
