...
from algorithms.DATAlign.embedding_model import DAT_Align as Multi_Order, StableFactor
from input.dataset import Dataset
from utils.graph_utils import load_gt
import torch.nn.functional as F
import torch.nn as nn
from algorithms.DATAlign.utils import *
from algorithms.DATAlign.losses import *
...
class DATAlign(NetworkAlignmentModel):
    """
    DATAlign model for networks alignment task
    """
    def __init__(self, source_dataset, target_dataset, args):
        ...
        """
        super(DATAlign, self).__init__(source_dataset, target_dataset)
        self.source_dataset = source_dataset
        self.target_dataset = target_dataset
        self.alphas = [args.alpha0, args.alpha1, args.alpha2]
        self.args = args
        self.full_dict = load_gt(args.groundtruth, source_dataset.id2idx, target_dataset.id2idx, 'dict')  #groundtruth

    def align(self):
        """
        The main function of DATAlign
        """
        #normlized laplacian
        #source_A_orbits, target_A_orbits, source_feats, target_feats = self.get_elements()
        source_L_orbits = get_L_orbits(self.source_dataset.get_adjacency_matrix())
        target_L_orbits = get_L_orbits(self.target_dataset.get_adjacency_matrix())
        source_feats, target_feats = self.get_feats()
        print("Running Multi-level embedding")
        DATAlign = self.multi_level_embed(source_A_hat, target_A_hat, source_feats, target_feats)
        # print("Running Refinement Alignment")
        # S_DATAlign = self.refinement_alignment(DATAlign, source_A_hat, target_A_hat)
        return S_DATAlign
    ...
    def multi_level_embed(self, source_A_hat, target_A_hat, source_feats, target_feats):
        ...
        """
        DATAlign = Multi_Order(
            activate_function = self.args.act,  #default = tanh
            num_GCN_blocks = self.args.num_GCN_blocks, #2
            input_dim = self.args.input_dim, #100
            output_dim = self.args.embedding_dim, #200
        ...
        if self.args.cuda:
            DATAlign = DATAlign.cuda()

        structural_optimizer = torch.optim.Adam(filter(lambda p: p.requires_grad, DATAlign.parameters()), lr=self.args.lr)
        ...
        for epoch in range(self.args.DATAlign_epochs): #10
            if self.args.log:
                print("Structure learning epoch: {}".format(epoch))
            for i in range(2):
                for j in range(len(new_source_A_hats)): #3
                    structural_optimizer.zero_grad()  #初始梯度清零
                    if i == 0:
                        A_hat = source_A_hat
                        augment_A_hat = new_source_A_hats[j]
                        outputs = DATAlign(source_A_hat, 's')
                        if j < 3:
                            augment_outputs = DATAlign(augment_A_hat, 's')
                        else:
                            augment_outputs = DATAlign(augment_A_hat, 's', new_source_feats)
                    else:
                        A_hat = target_A_hat
                        augment_A_hat = new_target_A_hats[j]
                        outputs = DATAlign(target_A_hat, 't')
                        if j < 3:
                            augment_outputs = DATAlign(augment_A_hat, 't')
                        else:
                            augment_outputs = DATAlign(augment_A_hat, 't', new_target_feats)
    ...
    def refinement_alignment(self, DATAlign, source_A_hat, target_A_hat):
        source_A_hat = source_A_hat.to_dense()  #计算laplacian时进行了sparse
        target_A_hat = target_A_hat.to_dense()
        DATAlign_S = self.refine(DATAlign, source_A_hat, target_A_hat, 0.94)
        return DATAlign_S
    ...
    def linkpred_loss(self, embedding, A):
        pred_adj = torch.matmul(F.normalize(embedding), F.normalize(embedding).t())  #S矩阵
        if self.args.cuda:
        ...
        linkpred_losss = linkpred_losss.sum() / A.shape[1]  ##为什么要除???
        return linkpred_losss


    def refine(self, DATAlign, source_A_hat, target_A_hat, threshold):
        refinement_model = StableFactor(len(source_A_hat), len(target_A_hat), self.args.cuda)
        if self.args.cuda: 
            refinement_model = refinement_model.cuda()
        S_max = None
        source_outputs = DATAlign(refinement_model(source_A_hat, 's'), 's')
        target_outputs = DATAlign(refinement_model(target_A_hat, 't'), 't')
        ...
        if 1:
            ...
            acc_max = acc
            S_max = S
        print("Before refinement, Acc: {}, score: {:.4f}".format(acc, score))
        ...
        DATAlign_S = S_max
        # self.log_and_evaluate(DATAlign, refinement_model, source_A_hat, target_A_hat)
        return DATAlign_S
    ...
    def get_similarity_matrices(self, source_outputs, target_outputs):
        ...
        """
        list_S = []
        for i in range(len(source_outputs)):  #逐层的H
