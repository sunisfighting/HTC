...
class GAlign(NetworkAlignmentModel):
    ...
    def __init__(self, source_dataset, target_dataset, args):
        ...
        # self.src_lap = torch.load(args.source_dataset + '/src_lap.pt')
        # self.trg_lap = torch.load(args.target_dataset + '/trg_lap.pt')
    ...
    def align(self):
        ...
        # output_s = GAlign(source_A_hat, 's')
        # output_t = GAlign(target_A_hat, 't')
        # S = torch.matmul(F.normalize(output_s[-1]), F.normalize(output_t[-1]).t())
        # print(S.shape)
        # S_GAlign = S.detach().cpu().numpy()
    ...
    def multi_level_embed(self, source_A_hat, target_A_hat, source_feats, target_feats):
        ...
        for epoch in range(self.args.GAlign_epochs): #10
            if self.args.log:
    ...
    def get_elements(self):
        ...
        # target_new = target_generate(0.4, self.target_dataset, type_aug='remove_edges')
